...
class YTDownloader:
    def __init__(self, pMetatag):
        self.metatag = pMetatag

    def download(self):
        pUrl = self.getUrl(self.metatag)
        pPath = self.downloadTrackWithPafy(pUrl, self.metatag)
        self.convertSong(pPath, self.metatag)
        self.fixMetaTags(pPath, self.metatag)
        return (pPath + '.mp3')[6:]

    def getUrl(self, pMetaTag):
        print(
            pMetaTag.getArtist().getArtistName() + " - " + pMetaTag.getTrack().getTrackName() + ": Fetching YouTube URL")
        query = (pMetaTag.getArtist().getArtistName() + ' - ' + pMetaTag.getTrack().getTrackName()).replace("Ä","Ae").replace("Ü","Ue").replace("Ö","Oe").replace("ä","ae").replace("ü","ue").replace("ö","oe").replace(" ",
                                                                                                            "+").replace(
            "&", "%26")
        ...
        for vid in soup.findAll(attrs={'class': 'yt-uix-tile-link'}):
            if vid['href'].find('channel') == -1 or vid['href'].find('googleads') == -1 or vid['href'].find(
                    'user') == -1:
    ...
    def convertSong(self, path, pMetaTag):
        ...
        subprocess.call(
            ["ffmpeg", "-hide_banner", "-loglevel", "panic", "-i", path + '.m4a', "-acodec", "libmp3lame", "-ab",
             "256k", path + '.mp3'])
        os.remove(path + '.m4a')

    def downloadTrackWithPafy(self, pUrl, pMetaTag):
        print(
            pMetaTag.getArtist().getArtistName() + " - " + pMetaTag.getTrack().getTrackName() + ": Downloading from YouTube")
